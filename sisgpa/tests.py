...
class ProyectoTest(TestCase):
    ...
    def test_permission_to_change_proyecto(self):
        c = self.client
        self.assertTrue(c.login(username='user', password='password'))
        pro = Proyecto.objects.first()
        response = c.get(reverse('project_update', args=(str(pro.id))))
...
class UserTest(TestCase):
    ...
    def test_edit_user(self):
        ...
        response = c.post('/users/{}/edit/'.format(u.id), {'username': 'nuevo_username', 'email': 'asd@asd.com'},
                          follow=True)
        # deberia redirigir
        self.assertRedirects(response, '/users/{}/'.format(u.id))
...
class RolesTest(TestCase):
    ...
    def test_create_role(self):
        ...
        response = c.post('/roles/add/', {'name': 'developer',
                                          'perms_proyecto': [u'add_proyecto', u'change_proyecto', u'delete_proyecto'],
                                          'perms_userstory': [u'add_userstory', u'delete_userstory']}, follow=True)
        # deberia redirigir
        self.assertEquals(response.status_code, 200)
    ...
    def test_not_create_invalid_role(self):
        ...
        response = c.post('/roles/add/',
                          {'name': '', 'perms_proyecto': [u'add_proyecto', u'change_proyecto', u'delete_proyecto'],
                           'perms_userstory': [u'add_userstory', u'delete_userstory']})
...
class FlujoTest(TestCase):
    def setUp(self):
        ...
        pro = Proyecto.objects.create(nombre_corto='Proyecto', nombre_largo='proyecto de test', estado='PE',
                                      fecha_inicio=timezone.now(),
    ...
    def test_permission_to_create_flujo(self):
        ...
        response = c.get(reverse('flujo_create', args=(str(p.id))))
...
class UserStoryTest(TestCase):
    def setUp(self):
        ...
        pro = Proyecto.objects.create(nombre_corto='Proyecto', nombre_largo='protexr asjd', estado='PE',
                                      fecha_inicio=timezone.now(),
                                      fecha_fin=timezone.now() + datetime.timedelta(days=30))
    ...
    def test_add_userstory_with_permission(self):
        ...
        # deberia existir
        self.assertIsNotNone(p)
        response = c.get(reverse('us_create', args=(str(p.id))))
        self.assertEquals(response.status_code, 200, 'No se pudo redirigir correctamente a Agreagar detail')
        response = c.post(reverse('us_create', args=(str(p.id))),
                          {'nombre': 'Test US', 'descripcion': 'This is a User Story for testing purposes.',
                           'prioridad': 1,
        ...
        response = c.get(reverse('us_detail', args=(str(us.id))))
    ...
    def test_update_userstory_with_permission(self):
        ...
        us = UserStory.objects.first()
        self.assertIsNotNone(us)
        self.assertEquals(us.nombre, 'Test US')
        response = c.get(reverse('us_detail', args=(str(us.id))))
        self.assertEquals(response.status_code, 200, 'No se pudo redirigir correctamente a userstory detail')
        # nos vamos a la pagina de edicion de user story
        response = c.get(reverse('us_update', args=(str(us.id))))
        # deberia retornar 200
        self.assertEquals(response.status_code, 200, 'No se pudo redirigir correctamente a editar user Story')
        response = c.post(reverse('us_update', args=(str(us.id))),
                          {'nombre': 'Test US2', 'descripcion': 'This is a User Story 2 for testing purposes.',
                           'prioridad': 1,
                           'valor_negocio': 10, 'valor_tecnico': 10, 'tiempo_estimado': 10}, follow=True)
        us = UserStory.objects.first()
        self.assertIsNotNone(us)
        ...
        self.assertNotEquals(us.nombre, 'Test US1')
        self.assertEquals(us.nombre, 'Test US2')
...
class SprintTest(TestCase):
    def setUp(self):
        ...
        pro = Proyecto.objects.create(nombre_corto='Proyecto', nombre_largo='Proyecto largo', estado='IN',
                                      fecha_inicio=timezone.now(),
                                      fecha_fin=timezone.now() + datetime.timedelta(days=30),
                                      fecha_creacion=timezone.now(), duracion_sprint='30',
